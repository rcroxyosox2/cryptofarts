...
// where to buy
const getTickers = async (coinId) => {
    const response = await fetch(`https://api.coingecko.com/api/v3/coins/${coinId}/tickers?include_exchange_logo=true`);
    const responseJson = await response.json();
    if (responseJson.error) {
      throw new Error(responseJson.error);
    }
...
const getCoinWithMarketChart = async (coinId, params = {
  ...
  return await CoinGeckoClient.coins.fetchMarketChart(coinId, params).then((response) => {
    ...
    // const pricesFormatted = prices.map(([date, price]) => [moment(date).format('L'), price]);
    const unique = (arrs = []) => {
      const keyed = (Array.isArray(arrs) ? arrs : []).map(([date, val]) => [moment(date).format('L'), val])
      return keyed.reduce((a, c) => {
        if (!a.find(v => v[0] === c[0])) {
          a.push(c);
        }
        return a;
    ...
    if (params.days > 1) {
      ...
      Object.keys(response.data).forEach((key) => {
        uniqueData[key] = unique(response.data[key]);
      });
      return uniqueData;
    }
