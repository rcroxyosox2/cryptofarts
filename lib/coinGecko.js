    ...
    .then((resp) => {
      ...
      if (data) {
        ...
        if (market_data && Object.keys(market_data).length) {
          ...
          .forEach((prop) => {
            filtered[prop] = (typeof market_data[prop] === 'object') ? market_data[prop][CURRENCY] : market_data[prop];
          })
...
const getMarketCapFromCoin = (coin) => {
  const marketCap = coin.market_data.market_cap;
  return (typeof marketCap === 'object') ? marketCap[CURRENCY] : marketCap;
}

const getATHPercentage = (coin) => {
  const perc = coin.market_data.ath_change_percentage;
  return (typeof perc === 'object') ? perc[CURRENCY] : perc;
}

const getATHDate = (coin) => {
  const athDate = coin.market_data.ath_date;
  return (typeof athDate === 'object') ? athDate[CURRENCY] : athDate;
}
